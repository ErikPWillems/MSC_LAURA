...
## Merging labratory data to the existing 07/08 dataset from the dietaryindex package

### Loading the dataset from 2007/2008 into R from the dietaryindex package
...
### Renaming the data to create a list / data frame with all PHDIs over the years

```{r}
PHDI_15_16_D1 <- PHDI_all_D1_Excess
```
...
```{r}
Blood_Pressure <- read.xport("~/Documents/Laura/UNI/Master/Master thesis/R_Scripts/MSC_LAURA/2015_2016/Examination_data_15:16/BPX_I.XPT")
Body_Measurements <- read.xport("~/Documents/Laura/UNI/Master/Master thesis/R_Scripts/MSC_LAURA/2015_2016/Examination_data_15:16/BMX_I.XPT")
F_Glucose <- read.xport("~/Documents/Laura/UNI/Master/Master thesis/R_Scripts/MSC_LAURA/2015_2016/Lab_data_15:16/GLU_I.XPT")
TRYGL <- read.xport("~/Documents/Laura/UNI/Master/Master thesis/R_Scripts/MSC_LAURA/2015_2016/Lab_data_15:16/TRIGLY_I.XPT")
CHOL <- read.xport("~/Documents/Laura/UNI/Master/Master thesis/R_Scripts/MSC_LAURA/2015_2016/Lab_data_15:16/TCHOL_I.XPT")
```
...
### Make a dataframe with the lab lists and PHDI combined

```{r}
PHDI_1516_df <- data.frame(
  SEQN = merged_list$FPED$SEQN,
  Gender = merged_list$FPED$RIAGENDR,
  Age = merged_list$FPED$RIDAGEYR,
  PHDI_1516 = PHDI_all_D1_Excess
)

df<- merge(PHDI_1516_df, Body_Measurements[, c("SEQN", "BMXWAIST")], by= "SEQN" )
...
```{r}
#PHDI with waist
ggplot(df, aes(PHDI_1516, BMXWAIST)) + geom_point(alpha= .2) + geom_smooth()

#PHDI with systolic high blood pressure
ggplot(df, aes(PHDI_1516, Systolic_mean)) + geom_point(alpha= .2) + geom_smooth()

#PHDI with diastolic high blood pressure
ggplot(df, aes(PHDI_1516, Diastolic_mean)) + geom_point() + geom_smooth()

#PHDI with low density lipoprotein LDL
ggplot(df, aes(PHDI_1516, LBDLDL)) + geom_point(alpha= .2) + geom_smooth()

# PHDI with the Glucose 
ggplot(df, aes(PHDI_1516, LBXGLU)) + geom_point(alpha= .2) + geom_smooth()

# PHDI with the fasting triglycerides
ggplot(df, aes(PHDI_1516, LBXTR)) + geom_point(alpha= .2) + geom_smooth()

ggplot(df, aes(LBXTR, LBDLDL)) + geom_point(alpha= .2) + geom_smooth()

summary(is.na(Blood_Pressure$BPAEN1))
summary(is.na(F_Glucose$LBXGLU))
```
