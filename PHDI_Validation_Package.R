#PHDI Calculation with data provided by package
...
load("~/Documents/Laura/UNI/Master/Master thesis/PHDI_VALIDATION.rda")
...
PHDI_V2 = function(SERV_DATA, RESPONDENTID, GENDER, TOTALKCAL_PHDI, WGRAIN_SERV_PHDI, STARCHY_VEG_SERV_PHDI, VEG_SERV_PHDI, 
  ...
  SERV_DATA %>%
    dplyr::mutate(
      ...
      PHDI_ALL = PHDI_WGRAIN + PHDI_STARCHY_VEG + PHDI_VEG + PHDI_FRT + PHDI_DAIRY + PHDI_REDPROC_MEAT + PHDI_POULTRY + 
        PHDI_EGG + PHDI_FISH + PHDI_NUTS + PHDI_LEGUMES + PHDI_SOY + PHDI_ADDED_FAT_UNSAT + PHDI_ADDED_FAT_SAT + 
        PHDI_ADDED_SUGAR
    ) %>%
...
PHDI_list = PHDI_V2(PHDI_VALIDATION, PHDI_VALIDATION$id, 
                    PHDI_VALIDATION$gender, PHDI_VALIDATION$TOTALKCAL_PHDI, 
                    PHDI_VALIDATION$WGRAIN_SERV_PHDI, PHDI_VALIDATION$STARCHY_VEG_SERV_PHDI, 
                    PHDI_VALIDATION$VEG_SERV_PHDI, PHDI_VALIDATION$FRT_SERV_PHDI, 
                    PHDI_VALIDATION$DAIRY_SERV_PHDI, PHDI_VALIDATION$REDPROC_MEAT_SERV_PHDI, 
                    PHDI_VALIDATION$POULTRY_SERV_PHDI, PHDI_VALIDATION$EGG_SERV_PHDI, 
                    PHDI_VALIDATION$FISH_SERV_PHDI, PHDI_VALIDATION$NUTS_SERV_PHDI, 
                    PHDI_VALIDATION$LEGUMES_SERV_PHDI, PHDI_VALIDATION$SOY_SERV_PHDI, 
...
hist(PHDI_list$PHDI_ALL)
mean(PHDI_list$PHDI_ALL) #74.12

# no we take some variables out and replace it with the NAs to see what happens with the code
PHDI_VALIDATION[2, "NUTS_SERV_PHDI"] <- NA
PHDI_VALIDATION[1, "NUTS_SERV_PHDI"] <- NA
PHDI_VALIDATION[9, "SOY_SERV_PHDI"] <- NA
PHDI_VALIDATION[15, "VEG_SERV_PHDI"] <- NA
PHDI_VALIDATION[3, "VEG_SERV_PHDI"] <- NA

PHDI_list_2 = PHDI_V2(PHDI_VALIDATION, PHDI_VALIDATION$id, 
                    PHDI_VALIDATION$gender, PHDI_VALIDATION$TOTALKCAL_PHDI, 
                    PHDI_VALIDATION$WGRAIN_SERV_PHDI, PHDI_VALIDATION$STARCHY_VEG_SERV_PHDI, 
                    PHDI_VALIDATION$VEG_SERV_PHDI, PHDI_VALIDATION$FRT_SERV_PHDI, 
                    PHDI_VALIDATION$DAIRY_SERV_PHDI, PHDI_VALIDATION$REDPROC_MEAT_SERV_PHDI, 
                    ...
                    PHDI_VALIDATION$ADDED_FAT_SAT_TRANS_SERV_PHDI, 
                    PHDI_VALIDATION$ADDED_SUGAR_SERV_PHDI)
hist(PHDI_list_2$PHDI_ALL)
mean(PHDI_list_2$PHDI_ALL, na.rm = T) #78.22

#so the mean goes up quite a lot, because the overall PHDI_all can not be calculated when there are NAs in the dietary data
# so therefore if we apply this code to the NHANES data with some NAs, these PHDI's are not calculated --> do we need to change the code,
#but then we will get lower scores probably....



PHDI_V2_changed = function(SERV_DATA, RESPONDENTID, GENDER, TOTALKCAL_PHDI, WGRAIN_SERV_PHDI, STARCHY_VEG_SERV_PHDI, VEG_SERV_PHDI, 
                           FRT_SERV_PHDI, DAIRY_SERV_PHDI, REDPROC_MEAT_SERV_PHDI, POULTRY_SERV_PHDI, EGG_SERV_PHDI, FISH_SERV_PHDI,
                           NUTS_SERV_PHDI, LEGUMES_SERV_PHDI, SOY_SERV_PHDI, ADDED_FAT_UNSAT_SERV_PHDI, ADDED_FAT_SAT_TRANS_SERV_PHDI,
                           ADDED_SUGAR_SERV_PHDI) {
  PHDI_MIN = 0
  ...
  PHDI_ALL = rowSums(
    cbind(
      ...
      SCORE_UNHEALTHY(STARCHY_VEG_SERV_PHDI, PHDI_MIN_STARCHY_VEG_SERV, PHDI_MAX_STARCHY_VEG_SERV, PHDI_MIN, PHDI_MAX),
      SCORE_HEALTHY(VEG_SERV_PHDI, PHDI_MIN_VEG_SERV, PHDI_MAX_VEG_SERV, PHDI_MIN, PHDI_MAX),
      SCORE_HEALTHY(FRT_SERV_PHDI, PHDI_MIN_FRT_SERV, PHDI_MAX_FRT_SERV, PHDI_MIN, PHDI_MAX),
      SCORE_UNHEALTHY(DAIRY_SERV_PHDI, PHDI_MIN_DAIRY_SERV, PHDI_MAX_DAIRY_SERV, PHDI_MIN, PHDI_MAX),
      SCORE_UNHEALTHY(REDPROC_MEAT_SERV_PHDI, PHDI_MIN_REDPROC_MEAT_SERV, PHDI_MAX_REDPROC_MEAT_SERV, PHDI_MIN, PHDI_MAX),
      SCORE_UNHEALTHY(POULTRY_SERV_PHDI, PHDI_MIN_POULTRY_SERV, PHDI_MAX_POULTRY_SERV, PHDI_MIN, PHDI_MAX),
      SCORE_UNHEALTHY(EGG_SERV_PHDI, PHDI_MIN_EGG_SERV, PHDI_MAX_EGG_SERV, PHDI_MIN, PHDI_MAX),
      SCORE_HEALTHY(FISH_SERV_PHDI, PHDI_MIN_FISH_SERV, PHDI_MAX_FISH_SERV, PHDI_MIN, PHDI_MAX),
      SCORE_HEALTHY(NUTS_SERV_PHDI, PHDI_MIN_NUTS_SERV, PHDI_MAX_NUTS_SERV, PHDI_MIN, PHDI_MAX),
      SCORE_HEALTHY(LEGUMES_SERV_PHDI, PHDI_MIN_LEGUMES_SERV, PHDI_MAX_LEGUMES_SERV, PHDI_MIN, PHDI_MAX_2),
      SCORE_HEALTHY(SOY_SERV_PHDI, PHDI_MIN_SOY_SERV, PHDI_MAX_SOY_SERV, PHDI_MIN, PHDI_MAX_2),
      SCORE_HEALTHY(ADDED_FAT_UNSAT_SERV_PHDI, PHDI_MIN_ADDED_FAT_UNSAT_SERV, PHDI_MAX_ADDED_FAT_UNSAT_SERV, PHDI_MIN, PHDI_MAX),
      SCORE_UNHEALTHY(ADDED_FAT_SAT_TRANS_SERV_PHDI, PHDI_MIN_ADDED_FAT_SAT_TRANS_SERV, PHDI_MAX_ADDED_FAT_SAT_TRANS_SERV, PHDI_MIN, PHDI_MAX),
      SCORE_UNHEALTHY(ADDED_SUGAR_SERV_PHDI, PHDI_MIN_ADDED_SUGAR_SERV, PHDI_MAX_ADDED_SUGAR_SERV, PHDI_MIN, PHDI_MAX)
    ),
    na.rm = TRUE
  )
  ...
  SERV_DATA %>%
    ...
    dplyr::select(RESPONDENTID, GENDER, PHDI_ALL, TOTALKCAL_PHDI)
}

PHDI_list_Changed = PHDI_V2_changed(PHDI_VALIDATION, PHDI_VALIDATION$id, 
                      PHDI_VALIDATION$gender, PHDI_VALIDATION$TOTALKCAL_PHDI, 
                      PHDI_VALIDATION$WGRAIN_SERV_PHDI, PHDI_VALIDATION$STARCHY_VEG_SERV_PHDI, 
                      PHDI_VALIDATION$VEG_SERV_PHDI, PHDI_VALIDATION$FRT_SERV_PHDI, 
                      PHDI_VALIDATION$DAIRY_SERV_PHDI, PHDI_VALIDATION$REDPROC_MEAT_SERV_PHDI, 
                      ...
                      PHDI_VALIDATION$ADDED_FAT_SAT_TRANS_SERV_PHDI, 
                      PHDI_VALIDATION$ADDED_SUGAR_SERV_PHDI)
hist(PHDI_list_Changed$PHDI_ALL)
mean(PHDI_list_Changed$PHDI_ALL, na.rm = T) #70
