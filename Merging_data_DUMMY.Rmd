...
```{r}
# Find the rows where the Food code number 256000 is present in the column 
rows_with_food_code <- merged_list$FPED_IND[merged_list$FPED_IND$DR1IFDCD == 92550110, ]
str(rows_with_food_code)

rows_with_food_code$Adjusted_DR1IGRMS <- rows_with_food_code$DR1IGRMS * (4.6/100)
str(rows_with_food_code)
...
for (i in 1:nrow(rows_with_food_code)) {
    ...
    merged_list$FPED$DR1T_ADD_SUGARS_GRAM[merged_list$FPED$SEQN == seqn] <- 
      merged_list$FPED$DR1T_ADD_SUGARS_GRAM[merged_list$FPED$SEQN == seqn] + rows_with_food_code[i, "Adjusted_DR1IGRMS"]
}   
print(merged_list$FPED$DR1T_ADD_SUGARS_GRAM)
```
...
print(PHDI_all_D1)
#--> SOMETHING IS NOT RIGHT, THERE ARE NEGATIVE VALUES IN THERE
```

### There were negative values in the PHDI_all_D1 list, which cant be true!

```{r}
# Function to check for negative values in a  list

check_negatives <- function(lst) {
  neg_values <- lst[!is.na(lst) & lst < 0]
  if (length(neg_values) > 0) {
    cat("There are negative values in the list.\n")
    print(neg_values)
  } else {
    cat("There are no negative values in the list.\n")
  }
}
check_negatives(PHDI_Seafood_Sub)
check_negatives(PHDI_Eggs)
check_negatives(PHDI_DGV_Ratio)
check_negatives(PHDI_ReV_Ratio)
print(PHDI_ReV_Ratio)
check_negatives(PHDI_all_D1)
