...
## Merging laboratory data to the existing NHANES2005_2006 dataset from the dietaryindex package
...
#to avoid duplications if the code is run several times
if ("DR1T_Total_Grams_Per_Day" %in% colnames(merged_list$FPED)) {
  merged_list$FPED$DR1T_Total_Grams_Per_Day <- NULL
}
...
```{r}
# Check if X exists in the variable
...
### There were negative values in the PHDI outcome, so there was a mistake --\> quickly looked at which function was wrong and then changed the ratio function !!
...
merged_list$FPED$DR1T_Total_Seafood_Sub_new <- ifelse(excess_N+merged_list$FPED$DR1T_Total_Seafood_Sub >=
                                                        PHDI_Seaf_S_max, PHDI_Seaf_S_max,
                                                      excess_N+merged_list$FPED$DR1T_Total_Seafood_Sub)
...
merged_list$FPED$DR1T_Total_Seafood_Sub_new <- ifelse(excess_L+merged_list$FPED$DR1T_Total_Seafood_Sub >=
                                                        PHDI_Seaf_S_max, PHDI_Seaf_S_max,
                                                      excess_L+merged_list$FPED$DR1T_Total_Seafood_Sub)
